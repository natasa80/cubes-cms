...
class Application_Model_DbTable_CmsIndexSlides extends Zend_Db_Table_Abstract {
    ...
    /**
     * 
     * @param array $indexSlide
     * @return int The new ID of new indexSlide (autoincrement)
     */
    public function insertIndexSlide($indexSlide) {
        ...
        if ($memebrWithBiggestOrderNumber instanceof Zend_Db_Table_Row) {
            ...
            $indexSlide['order_number'] = $memebrWithBiggestOrderNumber['order_number'] + 1;
        } else {
    ...
    public function deleteIndexSlide($id) {
        ...
        $indexSlidePhotoFilePath = PUBLIC_PATH . '/uploads/index_slides/' . $id . '.jpg';

        if (is_file($indexSlidePhotoFilePath)) {
            unlink($indexSlidePhotoFilePath);
        }

        //indexSlide to delete
    ...
    /**
     * 
     * @param nt $id ID of indexSlide to enable
     */
    ...
    public function updateIndexSlideOfOrder($sortedIds) {
        ...
        foreach ($filters as $field => $value) {
            ...
            switch ($field) {
                ...
                case 'title_search':
                    ...
                    $select->where('title_name LIKE ?', '%' . $value . '%');
                    break;
